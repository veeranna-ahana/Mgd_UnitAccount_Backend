const custMonthlyReportRouter = require("express").Router();
const { dailyReportQuery, setupQuery } = require("../../../../helpers/dbconn");
var bodyParser = require("body-parser");

//monthly report tax summary

custMonthlyReportRouter.post("/monthlyTaxSummary", async (req, res, next) => {
...
custMonthlyReportRouter.post(
  "/monthlyInvSummaryNames",
  async (req, res, next) => {
    const month = req.body.month;
    const year = req.body.year;
    try {
  ...
  async (req, res, next) => {
    ...
    try {
     ...
     FROM (
         ...
         WHERE YEAR(d1.Inv_Date) = '${year}' AND MONTH(d1.Inv_Date) = '${month}' AND d1.Cust_Name = '${name}'
...
//Machine Utilisation
custMonthlyReportRouter.post("/machineUtilaisation", async (req, res, next) => {
...
custMonthlyReportRouter.post("/allOutStandingBills", async (req, res, next) => {
  const month = req.body.month;
  const year = req.body.year;
  const name = req.body.getName;
  try {
...
//Collection Summary
custMonthlyReportRouter.post("/collectionSummary", async (req, res, next) => {
  const month = req.body.month;
  const year = req.body.year;
  const name = req.body.getName;
  try {
    dailyReportQuery(
     ...
     WHERE YEAR(p.Recd_PV_Date) = '${year}' AND MONTH(p.Recd_PV_Date) = '${month}' AND p.CustName = '${name}'
     GROUP BY p.TxnType
     UNION
     SELECT h.TxnType, SUM(h.Amount) AS Amount
     FROM magodmis.ho_paymentrv_register h
     WHERE YEAR(h.horefdate) = '${year}' AND MONTH(h.horefdate) = '${month}' AND  h.CustName = '${name}'
     GROUP BY h.TxnType;`,
      (err, data) => {
        res.send(data);
      }
...
//Clearance Summary
custMonthlyReportRouter.post("/clearanceSummary", async (req, res, next) => {
  const month = req.body.month;
  const year = req.body.year;
  const name = req.body.getName;
  try {
...
module.exports = custMonthlyReportRouter;
